# This script is based on https://github.com/aistairc/rnng-pytorch/blob/master/beam_search.py
...
# Data path options
parser.add_argument('--test_file', default='data/test_tokens.txt')
...
parser.add_argument(
    ...
    default=300,
    help=(
        'Number of tokens in a batch (batch_size*sentence_length) does not exceed this.\
        This value could be large value (e.g., 10000) safely\
        when --stack_size_bound is set to some value > 0. \
        Otherwise, we need to control (decrease) the batch size for longer sentences using this option,\
        because then stack_size_bound will grow by sentence length.'
    ),
)
parser.add_argument(
    '--stack_size_bound',
    ...
    default=-1,
    help=(
        'Stack size during search is bounded by this size.\
        If <= 0, the maximum size is 100. 100 looks sufficient for most of the grammars.\
        Bounding to some value (e.g., 100) is useful to reduce memory usage while increasing beam size.'
    ),
)
parser.add_argument(
    '--delay_word_ll',
...
def main(args):
    ...
    def dump_histroy(beam_history, sents):
        for pointer, bucket_i, beam, word_completed in beam_history:
            ...
            for batch_i in range(len(beam)):
                for _beam in beam[batch_i]:
                    print(
                        'beam: b_i: {}, {}'.format(
                            batch_i, _beam.dump(action_dict, sents[batch_i])
                        )
                    )
                for _beam in word_completed[batch_i]:
                    print(
    ...
    with torch.no_grad():
        ...
        block_parses = []
        block_surprisals = []
        # This is subword-based surprisal in general.
        # Conversion to word-level surprisal is done at output phase.
        ...
        for batch in tqdm(batches):
            ...
            if any(len(parse) == 0 for parse in parses):
                # parse failure (on some tree in a batch)
                failed_sents = [
                    (idx, " ".join(dataset.sents[idx].orig_tokens))
                    for parse, idx in zip(parses, batch_idx)
                    if len(parse) == 0
                ]
                failed_sents_str = "\n".join(
                    "{}: {}".format(idx, sent) for idx, sent in failed_sents
                )
                logger.warning(
            ...
            best_actions = [parse[0][0] for parse in parses]  # p[0][1] is likelihood
